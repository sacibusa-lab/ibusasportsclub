    ...
    <div class="bg-white rounded-3xl p-8 shadow-sm border border-zinc-100">
        <form id="articleForm" action="{{ route('admin.news.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" value="{{ $post->title }}" class="w-full bg-zinc-50 border border-zinc-100 p-4 rounded-xl font-black text-primary text-xl focus:ring-2 focus:ring-primary outline-none transition uppercase" placeholder="Enter headline..." required>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-4">
                        @if($post->image_url)
                        <img src="{{ $post->image_url }}" class="w-12 h-12 rounded-lg object-cover border border-zinc-100">
                        @endif
                        <input type="file" name="image" class="flex-1 bg-zinc-50 border border-zinc-100 p-3 rounded-xl font-bold text-primary focus:ring-2 focus:ring-primary outline-none transition text-xs" accept="image/*">
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between pt-6 border-t border-zinc-50">
                <label class="flex items-center gap-3 cursor-pointer group">
                    ...
                    <span class="text-[10px] font-black text-zinc-400 uppercase tracking-widest group-hover:text-primary transition">Published Status</span>
                </label>
