...
<body class="mesh-bg flex items-center justify-center min-h-screen p-6" x-data="{ showPass: false }">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-10">
            ...
            <p class="text-[10px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.3em] mt-2">Secure Management Gateway</p>
        </div>
        ...
        <div class="glass-panel rounded-[3rem] p-10 shadow-2xl relative overflow-hidden">
            ...
            <h2 class="text-[10px] font-black text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.2em] mb-10 border-l-4 border-secondary pl-4">Authorized Personnel Only</h2>
            
            <form action="{{ route('admin.login.post') }}" method="POST" class="space-y-6 relative">
                @csrf
                
                <div class="space-y-2">
                    <label class="text-[9px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.2em] px-1">Control Email</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <svg class="h-4 w-4 text-zinc-600 dark:text-zinc-300 dark:text-zinc-500 group-focus-within:text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        </div>
                        <input type="email" name="email" value="{{ old('email') }}" required autofocus
                            class="w-full bg-white/[0.02] border border-white/5 p-4 pl-12 rounded-2xl font-bold text-white focus:ring-2 focus:ring-secondary focus:bg-white/5 dark:bg-zinc-900/5 focus:border-transparent outline-none transition text-sm placeholder:text-zinc-700"
                            placeholder="admin@example.com">
                    </div>
                </div>

                <div class="space-y-2">
                    <label class="text-[9px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.2em] px-1">Access Token (Password)</label>
                    <div class="relative group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <svg class="h-4 w-4 text-zinc-600 dark:text-zinc-300 dark:text-zinc-500 group-focus-within:text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                        </div>
                        <input :type="showPass ? 'text' : 'password'" name="password" required
                            class="w-full bg-white/[0.02] border border-white/5 p-4 pl-12 rounded-2xl font-bold text-white focus:ring-2 focus:ring-secondary focus:bg-white/5 dark:bg-zinc-900/5 focus:border-transparent outline-none transition text-sm placeholder:text-zinc-700"
                            placeholder="••••••••">
                        <button type="button" @click="showPass = !showPass" class="absolute inset-y-0 right-0 pr-4 flex items-center">
                            <svg x-show="!showPass" class="h-4 w-4 text-zinc-600 dark:text-zinc-300 dark:text-zinc-500 hover:text-secondary transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                ...
                <div class="flex items-center justify-between px-2">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="remember" class="w-4 h-4 rounded-lg border-white/10 bg-white/5 dark:bg-zinc-900/5 text-secondary focus:ring-secondary transition-all">
                        <span class="text-[10px] font-black text-zinc-500 dark:text-zinc-400 group-hover:text-zinc-300 dark:text-zinc-500 transition uppercase tracking-widest">Maintain Session</span>
                    </label>
                </div>

                <button type="submit" 
                    class="w-full bg-secondary text-primary dark:text-white font-black py-5 rounded-2xl hover:bg-white dark:bg-zinc-900 transition-all duration-500 uppercase tracking-[0.2em] text-[10px] shadow-2xl flex items-center justify-center gap-3 hover:-translate-y-1">
        ...
        <div class="mt-12 text-center">
            <a href="{{ route('home') }}" class="text-[9px] font-black text-zinc-600 hover:text-zinc-400 dark:text-zinc-500 dark:text-zinc-400 transition uppercase tracking-[0.3em] flex items-center justify-center gap-2">
