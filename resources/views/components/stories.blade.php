<div x-data="{ 
    ...
    <!-- Stories Carousel -->
    <h2 class="text-xl md:text-2xl font-black text-primary uppercase tracking-tighter mb-6 px-4 sm:px-0">Stories</h2>
    <div class="flex gap-3 overflow-x-auto no-scrollbar pb-2">
        @foreach($stories as $index => $story)
            @continue($story->items->count() == 0)
            <button @click="openStory({{ $index }})" class="flex-none group focus:outline-none">
                <div
                    class="relative w-32 h-48 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105">
                    @php
                        $firstItem = $story->items->first();
                        $previewUrl = $story->thumbnail_url ?? $firstItem->media_url;
                        $isVideo = $previewUrl === $firstItem->media_url && $firstItem->type === 'video';
                    @endphp

                    @if($isVideo)
                        <video src="{{ $previewUrl }}#t=0.001" class="w-full h-full object-cover" muted playsinline
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-3">
                        <span
                            class="text-xs font-black text-white uppercase tracking-tight line-clamp-2 drop-shadow-lg">{{ $story->title }}</span>
                    </div>
    ...
    <template x-teleport="body">
        <div x-show="showViewer" class="fixed inset-0 z-[100] bg-black flex items-center justify-center select-none"
            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
            x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-200"
            x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95"
            @keydown.escape.window="closeViewer()" style="display: none;">

            <div
                class="w-full h-full max-w-lg relative flex flex-col bg-zinc-900 md:rounded-3xl overflow-hidden shadow-2xl">
                <!-- Close Button -->
                <button @click="closeViewer()"
                    class="absolute top-8 right-6 z-[120] text-white/50 hover:text-white transition p-2 bg-black/20 rounded-full backdrop-blur-sm">
                ...
                <div class="absolute top-10 left-6 z-[115] flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full border border-white/20 overflow-hidden">
                        <img :src="groups[groupIndex]?.thumbnail_url ?? groups[groupIndex]?.items[0]?.media_url"
                            class="w-full h-full object-cover">
                    </div>
                    <div class="flex flex-col">
                        <span class="text-white text-[10px] font-black uppercase tracking-widest"
                            x-text="groups[groupIndex]?.title"></span>
                        <span class="text-white/40 text-[8px] font-bold"
                            x-text="(itemIndex + 1) + ' of ' + groups[groupIndex]?.items.length"></span>
                    </div>
                </div>
                ...
                <div class="flex-1 relative flex items-center justify-center overflow-hidden">
                    <template x-for="(item, idx) in groups[groupIndex]?.items" :key="idx">
                        <div x-show="itemIndex === idx" class="w-full h-full flex items-center justify-center">
                            ...
                            <video x-show="item.type === 'video'" :src="item.media_url" x-ref="storyVideo" autoplay
                                muted playsinline preload="auto" class="w-full h-full object-cover" @ended="nextItem()"
                                @play="startItem()" @pause="if (timer) clearInterval(timer)" style="object-fit: cover;">
                            </video>
                        </div>
                    </template>
                    ...
                    <div class="absolute inset-0 z-[100] flex">
                        ...
                        <div class="w-1/3 h-full" @mousedown="if (timer) clearInterval(timer)" @mouseup="startItem()">
                        </div>
                    ...
                    <div class="absolute top-24 right-6 z-[130] flex flex-col gap-3" @click.stop>
                        ...
                        <button @click="toggleMute()" type="button"
                            ...
                            <svg x-show="!isMuted" class="w-6 h-6 text-white" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                        ...
                        <!-- Play/Pause Button -->
                        <button @click="togglePlayPause()" type="button"
                            class="w-12 h-12 bg-black/60 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-black/80 transition shadow-xl">
                            <svg x-show="!isPlaying" class="w-6 h-6 text-white ml-1" fill="currentColor"
                                viewBox="0 0 24 24">
                    ...
                    <div class="absolute bottom-0 left-0 right-0 p-8 z-[115]">
                        <template x-if="groups[groupIndex]?.link_url || groups[groupIndex]?.items[itemIndex]?.link_url">
                            <div class="flex flex-col items-center gap-4">
                                <a :href="groups[groupIndex]?.link_url || groups[groupIndex]?.items[itemIndex]?.link_url"
                                    target="_blank"
                                    class="w-full bg-[#0066FF] text-white font-black py-4 rounded-xl uppercase text-[11px] tracking-widest hover:bg-white hover:text-[#0066FF] transition duration-300 shadow-2xl flex items-center justify-center gap-2">
